...
| AdSense | ⚠️ **재심사 중** | 3차 거절 (가치 없는 콘텐츠) → 니치 전환 후 재도전 |
...
## 2026-02-25 TODO (내일 작업)

### 1. 검색 인덱스 draft 필터링 수정
| 작업 | 상세 | 우선순위 |
|------|------|---------|
| `scripts/generate-search-index.js` 수정 | draft: true 글 제외하도록 필터 추가 (현재 118개 전부 포함됨) | 높음 |
| 사이트 내 검색 테스트 | /search 에서 draft 글이 검색되지 않는지 확인 | 높음 |

### 2. 사이트 정체성 재정비
| 작업 | 상세 | 우선순위 |
|------|------|---------|
| 홈페이지 히어로/설명 수정 | "유용한 정보" → "생활 금융 계산기 & 가이드" 등 금융 니치 반영 | 높음 |
| About 페이지 수정 | 금융/보험 전문 사이트로 정체성 명확화 | 높음 |
| 네비게이션 정리 | 카테고리를 finance + insurance만 표시, 불필요한 메뉴 정리 | 중간 |
| 푸터 설명 수정 | 사이트 소개 문구를 금융 니치에 맞게 변경 | 중간 |

### 3. 계산기/도구 정리
| 작업 | 상세 | 우선순위 |
|------|------|---------|
| 금융 외 도구 검토 | BMI, 수면, 알코올 등 금융과 무관한 도구 숨김 검토 | 중간 |
| 도구 페이지 카테고리 정리 | 금융 계산기 중심으로 재배치 | 낮음 |

### 4. 테스트/퀴즈 정리
| 작업 | 상세 | 우선순위 |
|------|------|---------|
| 금융 외 테스트 검토 | 수면유형, 카페인, SNS피로도 등 금융과 무관한 테스트 숨김 검토 | 중간 |
| 재테크성향, 소비유형 테스트는 유지 | 금융 니치와 일치 | - |

### 5. AdSense 재신청
| 작업 | 상세 | 우선순위 |
|------|------|---------|
| 위 작업 완료 후 사이트 전체 점검 | 금융/보험 25개 글 + 금융 계산기만 보이는지 확인 | 높음 |
| AdSense 재신청 | 사이트 정체성 변경 반영 확인 후 신청 | 높음 |

### 참고: 현재 상태
- **공개 글**: 25개 (finance 18 + insurance 7)
- **draft 글**: 93개 (파일 보존, `draft: true`로 숨김)
- **변경 파일**: `src/lib/posts.ts` (draft 필터 추가), 93개 md 파일
